...
Handler.prototype.middlewarify = function () {
  ...
  return function (req, res, next) {
    ...
    if (config.encodingAESKey) {
        that.cryptor = new WXBizMsgCrypt(config.token, config.encodingAESKey, config.appid);
    }
    ...
    if (req.query.encrypt_type && req.query.msg_signature) {
      ...
      if (that.checkSignature && !checkSignature(req.query, req.wechat_token || config.token)) {
        res.writeHead(401);
        res.end('Invalid signature');
        return;
      }
